...
describe('Household', () => {
    before((done) => { // At the start of the test we populate the test database
        Household.insertMany(household_list, (err: any) => {
            done();
        });
    });
    after((done) => { // At the end of the test we drop the test database
        Household.collection.drop((err: any) => {
            done();
        });
    });
